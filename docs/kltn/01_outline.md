...
- **Nội dung:** Năm ngoại vi gắn TileLink-UL; cơ chế endpoint MMIO; cách `javascript.js` nối sự kiện UI (bàn phím/chuột/console/CAN inject frame) vào ngoại vi.
- **Nguồn:** `src/js/soc.js`; `src/js/javascript.js`.
- **Hình/bảng:** **Hình 4.11 – Kiến trúc ngoại vi** *(hình bắt buộc #6)* (UL → UART/CAN/LED/Keyboard/Mouse + đường sự kiện từ UI). **Bảng 4.9 – Danh sách ngoại vi hỗ trợ** *(bảng bắt buộc #5)* (tên, base, kích thước, mô tả).
- **Trang:** ~1.5.
...
- **Nội dung:** Base `0xFF200000`, range `0x100`; standard ID 11-bit, DLC 0..8, payload 8 byte, một TX mailbox, một RX mailbox, loopback; register map và giới hạn frame/message-level.
- **Nguồn:** `src/js/can.js`, `src/js/soc.js`; demo/test `test/can_loopback.asm`, `test/can_verify.mjs`, `test/can_mmio_verify.mjs`.
- **Hình/bảng:** **Bảng 4.11** – Bản đồ thanh ghi CAN.
- **Trang:** ~1.5.
...
- **Hình/bảng:** **Bảng 4.12** – Bản đồ thanh ghi Keyboard & Mouse.
- **Trang:** ~1.

> **Giới hạn CAN:** Chỉ mô tả là controller giáo dục ở mức frame/message qua MMIO. Không viết là mô phỏng bit-level/physical-layer đầy đủ hoặc tuân thủ đầy đủ ISO 11898.
...
- **Nội dung:** Bổ sung CSR/privileged, FCSR/làm tròn động, thực thi FENCE; mở rộng TileLink B/C/E + coherence; arbitration đa master thực sự; interrupt controller + IRQ cho các khối phù hợp; mở rộng CAN theo hướng nhiều nút/mô hình lỗi chi tiết; benchmark định lượng; bài lab giảng dạy.
...
5. **CAN:** có trong source ở mức frame/message qua MMIO; giữ rõ giới hạn, không overclaim bit-level/physical layer.
