...
Việc mô phỏng một hệ thống tính toán có thể được thực hiện ở nhiều mức trừu tượng khác nhau, mỗi mức đánh đổi giữa độ chi tiết và tốc độ mô phỏng [CẦN NGUỒN]. Phân biệt các mức này giúp xác định rõ những gì một trình mô phỏng tái hiện và những gì nằm ngoài phạm vi của nó.
...
Tập lệnh cơ sở RV32I định nghĩa 32 thanh ghi số nguyên rộng 32 bit, ký hiệu x0 đến x31, cùng một bộ đếm chương trình (Program Counter – PC) trỏ tới lệnh đang thực thi [6]. Thanh ghi x0 được nối cứng giá trị 0: mọi thao tác đọc x0 đều trả về 0 và mọi thao tác ghi vào x0 đều không có hiệu lực; đặc điểm này được dùng để biểu diễn nhiều thao tác thông dụng một cách gọn gàng. Các thanh ghi còn lại có công dụng chung, với quy ước sử dụng (lưu địa chỉ trả về, con trỏ ngăn xếp, tham số hàm…) được quy định bởi giao diện nhị phân ứng dụng (ABI); ánh xạ tên thanh ghi theo ABI được trình bày ở Chương 4.
...
TileLink định nghĩa ba cấp độ tuân thủ, tăng dần về mức độ phức tạp và tập thao tác hỗ trợ [7]:
...
Giao thức tổ chức việc truyền tin trên tối đa **năm kênh** một chiều, ký hiệu A đến E, với vai trò như sau [7]:
...
*Ghi chú: Mã số opcode theo định nghĩa trong source `src/js/tilelink.js`, nhất quán với đặc tả TileLink [7]. Các opcode kênh D thuộc nhóm cấp quyền (`Grant`/`GrantData`/`ReleaseAck`) liên quan đến cơ chế nhất quán của TL-C, được định nghĩa để tham chiếu nhưng không được sử dụng trong luồng giao dịch của hệ thống.*
...
**Thư viện soạn thảo CodeMirror.** CodeMirror là một thư viện soạn thảo mã nguồn chạy trong trình duyệt, cung cấp các tính năng như tô màu cú pháp theo ngôn ngữ, hiển thị số dòng và hỗ trợ gợi ý khi nhập [12]. Thư viện này thường được dùng để xây dựng vùng nhập mã trong các công cụ lập trình trên nền web.
...
> *Ghi chú: Các mục [6]–[9] đã được thiết lập ở Chương 1 và được dùng lại trong chương này; chúng được liệt kê lại dưới đây để tiện theo dõi. Các mục [10]–[13] là nguồn mới bổ sung cho Chương 2. Toàn bộ danh sách sẽ được hợp nhất và chuẩn hóa theo IEEE ở mục Tài liệu tham khảo chung của báo cáo; cần rà soát phiên bản, năm xuất bản và ngày truy cập khi hoàn thiện.*
...
[10] D. A. Patterson và J. L. Hennessy, *Computer Organization and Design: The Hardware/Software Interface, RISC-V Edition*. Cambridge, MA, USA: Morgan Kaufmann. *(cần ghi rõ phiên bản và năm khi hoàn thiện)*

[11] J. L. Hennessy và D. A. Patterson, *Computer Architecture: A Quantitative Approach*, 6th ed. Cambridge, MA, USA: Morgan Kaufmann, 2019.

[12] M. Haverbeke, "CodeMirror," thư viện soạn thảo mã trên nền web. [Trực tuyến]. Có tại: https://codemirror.net/ *(cần ghi rõ phiên bản và ngày truy cập khi hoàn thiện)*

[13] "MDN Web Docs," Mozilla. [Trực tuyến]. Có tại: https://developer.mozilla.org/ *(nguồn tham khảo cho HTML, CSS, JavaScript, ES modules và phần tử canvas; cần ghi rõ ngày truy cập khi hoàn thiện)*
