...
| `src/js/uart.js`, `can.js`, `led_matrix.js`, `keyboard.js`, `mouse.js` | Năm thiết bị ngoại vi |
...
|---|---|---|
| Kiểm thử đơn vị/khói (Node.js) | `assembler_verify.mjs`, `asm_programs_verify.mjs`, `mmu_basic_verify.mjs`, `tilelink_verify.mjs`, `dma_verify.mjs`, `can_verify.mjs`, `can_mmio_verify.mjs`, `syscall_output_verify.mjs`, `mmu_syscall_verify.mjs`, `log_filter_verify.mjs` | Kiểm tra cục bộ từng mô-đun và tích hợp nhẹ |
...
Hệ thống có năm thiết bị ngoại vi gắn vào TileLink-UL: UART, bộ điều khiển CAN, ma trận LED, bàn phím và chuột. Mỗi thiết bị được trình diện trên bus qua một **endpoint MMIO** tạo bởi hàm `createMMIOEndpoint` (xem source: `src/js/soc.js`): endpoint này nhận yêu cầu bus, gọi hàm đọc/ghi tương ứng của thiết bị, rồi trả phản hồi `AccessAckData`/`AccessAck`. Cách bố trí này tách bạch logic thiết bị (trong từng mô-đun ngoại vi) khỏi việc đấu nối bus. Bảng 4.9 liệt kê năm thiết bị ngoại vi cùng vùng địa chỉ; để tham chiếu đầy đủ, vùng thanh ghi DMA cũng được liệt kê dù DMA không phải thiết bị ngoại vi theo nghĩa thông thường.
...
[Hình 4.11: Kiến trúc hệ thống ngoại vi và đường sự kiện từ giao diện. Trung tâm là khối bus ngang "TileLink-UL". Từ bus tỏa xuống năm khối ngoại vi: "UART (0x10000000)", "LED Matrix (0xFF000000)", "Mouse (0xFF100000)", "CAN (0xFF200000)" và "Keyboard (0xFFFF0000)", mỗi khối nối với bus qua một ô nhỏ "MMIO endpoint (createMMIOEndpoint)". Phía trên bus UL, một mũi tên hai chiều "Bridge ↔ TileLink-UH" thể hiện UL nối lên bus UH. Bên phải là cột giao diện web với các đường sự kiện cho bàn phím, chuột, UART và thao tác bơm frame nhận cho CAN. Ghi nhãn ở mỗi đường nối bus mô tả nhóm thanh ghi chính của thiết bị.] (xem source: `src/js/soc.js`, `src/js/javascript.js`)
...
**Bảng 4.11. Bản đồ thanh ghi CAN**
...
| `io` | Chứa "uart"/"can controller"/"keyboard"/"mouse"/"led matrix"/"io map" |
