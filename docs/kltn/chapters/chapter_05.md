...
Đối với CAN, `can_verify.mjs` kiểm trực tiếp register map, loopback, frame chuẩn/mở rộng, lỗi DLC/identifier, RX overrun, TX backpressure và định tuyến SoC qua TileLink-UL/cầu nối; `can_mmio_verify.mjs` biên dịch và chạy `can_loopback.asm` qua CPU, xác nhận đường MMU không lưu đệm, cache bypass, cầu nối UH→UL và endpoint CAN. Kết quả lượt rà soát cuối: cả hai script CAN đều pass; chương trình loopback kết thúc với mã 0 sau 475 chu kỳ mô phỏng. `asm_programs_verify.mjs` cũng biên dịch thành công 19/19 chương trình mẫu, trong đó `can_loopback.asm` có 59 lệnh; `log_filter_verify.mjs` pass, còn logic classifier hiện nhận nhãn CAN vào nhóm I/O (xem source: `test/can_verify.mjs`, `test/can_mmio_verify.mjs`, `test/can_loopback.asm`, `test/asm_programs_verify.mjs`, `test/log_filter_verify.mjs`).
